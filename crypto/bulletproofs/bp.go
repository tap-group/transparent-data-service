...
func Prove(secret *big.Int, params BulletProofSetupParams, gamma *big.Int) (BulletProof, error) {
	...
	V, _ := p256.CommitG1(secret, gamma, params.H)

	// aL, aR and commitment: (A, alpha)
	aL, _ := util.Decompose(secret, 2, params.N)                               // (41)
	...
	*/
	...
	// sp1 + sp2
	t1 := util.Add(sp1, sp2)
	t1 = util.Mod(t1, ORDER)

	// compute t2: < sL, y^n . sR >
	t2, _ := ScalarProduct(sL, ynsR)
	t2 = util.Mod(t2, ORDER)

	// compute T1
	T1, _ := p256.CommitG1(t1, tau1, params.H) // (53)

	// compute T2
	T2, _ := p256.CommitG1(t2, tau2, params.H) // (53)
	...
	taux := util.Multiply(tau2, util.Multiply(x, x))
	taux = util.Add(taux, util.Multiply(tau1, x))
	taux = util.Add(taux, util.Multiply(util.Multiply(z, z), gamma))
	taux = util.Mod(taux, ORDER)

	// Compute mu = alpha + rho.x                                          // (62)
	mu := util.Multiply(rho, x)
	mu = util.Add(mu, alpha)
	mu = util.Mod(mu, ORDER)
...
func (proof *BulletProof) Verify() (bool, error) {
	...
	lhs, _ := p256.CommitG1(proof.Tprime, proof.Taux, params.H)

	// Compute right hand side
	z2 := util.Multiply(z, z)
	z2 = util.Mod(z2, ORDER)
	x2 := util.Multiply(x, x)
	x2 = util.Mod(x2, ORDER)
...
func updateGenerators(Hh []*p256.P256, y *big.Int, N int64) []*p256.P256 {
	...
	// Switch generators
	yinv := util.ModInverse(y, ORDER)
	expy := yinv
	hprime[0] = Hh[0]
	i = 1
	for i < N {
		hprime[i] = new(p256.P256).ScalarMult(Hh[i], expy)
		expy = util.Multiply(expy, yinv)
		i = i + 1
	}
	return hprime
}
...
func (params *BulletProofSetupParams) delta(y, z *big.Int) *big.Int {
	...
	z2 := util.Multiply(z, z)
	z2 = util.Mod(z2, ORDER)
	z3 := util.Multiply(z2, z)
	z3 = util.Mod(z3, ORDER)
	...
	result = util.Sub(z, z2)
	result = util.Mod(result, ORDER)
	result = util.Multiply(result, sp1y)
	result = util.Mod(result, ORDER)
	result = util.Sub(result, util.Multiply(z3, sp12))
	result = util.Mod(result, ORDER)
